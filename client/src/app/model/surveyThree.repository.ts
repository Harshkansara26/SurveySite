 ...
 */
 ...
 export class SurveyThreeRepository
 {
   ...
   deleteSurveyThreeBy(id: string) {
     this.dataSource.deleteSurveyThree(id).subscribe(response => {
       const updatedSurveyThree = this.surveyThree.filter(response => response._id !== response);
       this.surveyThree = [...updatedSurveyThree]
     });
   }
